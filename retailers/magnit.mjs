const PACK_PATTERNS = [
  { unit: "pcs", re: /(\d+(?:[.,]\d+)?)\s*(?:шт|pcs|пак(?:ет(?:ик)?(?:ов|а)?)?)(?![a-zа-яё])/i, factor: 1 },
...
function number(value){if(value==null||value==="")return null;if(typeof value==="number")return Number.isFinite(value)?value:null;const parsed=Number(String(value).replace(/\s/g,"").replace(",",".").replace(/[^\d.-]/g,""));return Number.isFinite(parsed)?parsed:null;}
export function parseMagnitPack(text){const source=String(text||"");for(const pattern of PACK_PATTERNS){const match=source.match(pattern.re);if(!match)continue;const value=number(match[1]);if(value==null)continue;if(pattern.unit==="kg")return{value:value*pattern.factor,unit:"g",source:match[0]};if(pattern.unit==="l")return{value:value*pattern.factor,unit:"ml",source:match[0]};return{value:value*pattern.factor,unit:pattern.unit,source:match[0]};}return null;}
export function resolveMagnitPack(raw){
  const titlePack=parseMagnitPack(raw&&raw.name);
  const observedPack=raw&&raw.pack&&Number.isFinite(Number(raw.pack.value))&&Number(raw.pack.value)>0?raw.pack:null;
