...
export function adaptLentaCatalog(rows=[],context={}){
  ...
  return rows.map(row=>({
    ...
    store_context:store?{store_code:store.store_code?String(store.store_code):null,address:store.address||null}:null,
    catalog_context:context.catalog_context||null,
    source:{
      kind:context.method==="aggregator_catalog_snapshot"?"aggregator_catalog":"retailer_catalog",
      name:context.method==="aggregator_catalog_snapshot"?"proshoper":"lenta",
