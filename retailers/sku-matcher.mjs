...
export function buildPriceOverlay(products, options={}) {
  ...
  for (const product of products||[]) {
    ...
    const candidate={sku:result.sku,name:product.name,retailer_product_id:product.retailer_product_id||null,price_rub:product.price_rub,old_price_rub:Number.isFinite(product.old_price_rub)?product.old_price_rub:null,promo:Boolean(product.promo),promo_eligibility_verified:product.promo_eligibility_verified===true,promo_terms_verified:product.promo_terms_verified===true,source_url:product.source_url||null,...(product.image_url?{image_url:product.image_url}:{}),confidence:result.confidence,method:result.method,comparison_eligible:true,source_pack:eligibility.source_pack,requested_pack:eligibility.requested_pack,availability:product.availability};
