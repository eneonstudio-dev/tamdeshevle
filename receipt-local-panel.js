(function () {
  ...
  function api() {
    return window.TDReceiptLocal || null;
  }
  ...
  function renderList(root) {
    ...
    list.innerHTML = drafts.slice(0, 20).map(draft => {
      ...
      return `<div class="receipt-local-row">
        <div><b>${esc(title)}</b><small>${esc(summary.store_id || "магазин")} · ${esc(formatDate(summary.observed_at))}</small></div>
  ...
  function openPanel() {
    ...
    overlay.innerHTML = `<section class="receipt-local-sheet" role="dialog" aria-modal="true" aria-label="Черновики чеков">
      ...
      <div class="receipt-local-actions">
        <button type="button" data-action="export">Скачать JSON-копию</button>
        <label>Импорт JSON<input type="file" accept="application/json,.json" data-action="import" hidden></label>
      </div>
      <div class="receipt-local-status" hidden></div>
      <div class="receipt-local-list"></div>
      <button type="button" class="receipt-local-clear" data-action="clear">Очистить локальные черновики</button>
      <div class="receipt-local-foot">JSON-копия — только резервная копия. Импортированные чеки остаются <b>rankable:false</b> и не становятся подтверждённой ценой.</div>
    </section>`;
    ...
    overlay.querySelector('[data-action="export"]').addEventListener("click", () => {
      const count = tools.downloadExport();
      status.hidden = false;
      status.textContent = `Сохранила JSON-копию: ${count} чек(ов).`;
    });
    ...
    overlay.querySelector('[data-action="import"]').addEventListener("change", async event => {
      ...
      if (!file) return;
      try {
        const result = await tools.importFile(file);
        status.hidden = false;
        status.textContent = `Импорт: ${result.imported}; пропущено: ${result.rejected}. В рейтинг ничего не попало.`;
        renderList(overlay);
      } catch (_) {
        status.hidden = false;
        status.textContent = "Не смогла прочитать JSON-файл.";
      }
      event.target.value = "";
    });
    ...
    overlay.querySelector('[data-action="clear"]').addEventListener("click", () => {
      ...
      tools.clearDrafts();
      status.hidden = false;
      status.textContent = "Локальные черновики удалены.";
      renderList(overlay);
    });
  }
  ...
  function mount() {
    ...
    button.type = "button";
    button.className = "receipt-local-open";
  ...
  window.TDReceiptLocalPanel = { open: openPanel, mount };
})();