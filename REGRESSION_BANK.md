...
| `REG-019` | P1 | MVP-021: user says `Добавь две пачки макарон`, but the deterministic parser recognizes only digit quantities and silently loses the word-form amount | Parse supported Russian cardinal word quantities into explicit `SET_PRODUCT_AMOUNT`; preserve quantity 2 through ShoppingState and PurchasePlan totals instead of falling back to one pack | intent/state/optimizer | OPEN | `scripts/test-mvp021-word-quantity.mjs` | PR #393 |
| `REG-020` | P1 | A grocery title contains a long retailer/brand token on a narrow screen | Keep the full decision-critical name readable in Bay, comparison and retailer handoff; wrap safely without widening cards, colliding with prices/actions or hiding text | UX/handoff | OPEN | `scripts/test-roxy-long-product-names.py` | PR #395 |
