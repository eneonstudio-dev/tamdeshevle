...
| `REG-018` | P1 | User asks `Почему этот вариант?` after Bay/optimizer produced a shopping decision | Route the follow-up into the grounded decision explanation and derive the answer from current PurchasePlan totals, actual constraints, evidence quality and known/unknown fulfillment trade-offs; never fall through to generic invented reasoning | intent/explanation | GUARDED | `scripts/test-bai-explanation.mjs` | PR #389 |
