...
| `REG-023` | P1 | MVP-022: after pasta quantity is 2, user says `Удали одну пачку макарон` | Decrement pasta by exactly one pack, keep the SKU at quantity 1, preserve unrelated basket lines and synchronize PurchasePlan totals; quantity-less remove still deletes the SKU | intent/state/optimizer | GUARDED | `scripts/test-mvp022-decrement-pack.mjs` | PR #397 |
| `REG-024` | P1 | MVP-023: user says `Бюджет теперь 4000` after an existing constrained basket | Parse the explicit budget update deterministically even without a currency suffix, change only the budget dimension, preserve retailer/people/duration/products/quantities/brand constraints, and re-evaluate PurchasePlan | intent/state/optimizer | GUARDED | `scripts/test-mvp023-budget-only.mjs` | PR #399 |
| `REG-025` | P1 | Magnit renders a loyalty-gated price (`Цена по карте` / explicit Premium price), or separately advertises an extra Premium benefit near a normal current price | Only explicit card/Premium-gated prices become conditional and non-rankable until eligibility + terms are verified; a normal displayed base price must not be suppressed merely because `+N% с Премиум` is advertised separately | truth | GUARDED | `scripts/test-magnit-collector.mjs` + `scripts/test-magnit-adapter.mjs` | PR #404 + live collector #13 |
| `REG-026` | P1 | MVP-024: user says `Бренды не важны` after Bay already holds excluded-brand constraints | Accept the short shopping follow-up through Bay Domain Gate, clear brand constraints in legacy ShoppingState and canonical Bay session, preserve basket/budget/store/people/duration/quantities, and re-evaluate a verified PurchasePlan | intent/state/runtime | OPEN | `scripts/test-mvp024-brand-relax.mjs` | PR #406 |
