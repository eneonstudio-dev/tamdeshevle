(()=>{
  ...
  function project(base,ops,text=""){
    ...
    const t=low(text);
    if(/\b(?:я\s+один|мне\s+одному|для\s+себя)\b/.test(t))s.peopleCount=1;
    return s;
  }
  ...
  function assumptions(state,routed){
    const explicit=new Set((routed?.operations||[]).map(op=>op?.type));
    const bits=[];
  ...
  async function autoPlan(text,routed){
    ...
    const base=window.TDShoppingState?.get?.()||{},scenario=project(base,routed?.operations||[],text);
    const pack=planner.build(scenario,text),chosen=pack?.recommended;
    if(!chosen?.operations?.length)return routed;
    const operations=mergeOps(routed?.operations,chosen.operations);
    if(!operations.length)return routed;
