(()=>{
  ...
  function grounded(raw,op){
    const t=low(raw),edge="(?:^|[\\s,])",tail="(?=$|[\\s,.!?])";
    if(op.type==="CHANGE_BUDGET")return /(?:бюджет|до|максимум|на)\s*\d[\d\s.,]*\s*(?:р|₽|руб)/.test(t);
    if(op.type==="SET_PEOPLE")return /(?:человек|чел\.?|персон|едок)/.test(t)||new RegExp(edge+"нас\\s+(?:\\d+|один|одна|два|две|двое|три|трое|четыре|четверо)"+tail).test(t)||new RegExp(edge+"(?:я\\s+один|мне\\s+одному|для\\s+себя)"+tail).test(t);
    if(op.type==="SET_DURATION")return /(?:на|примерно на)\s+(?:\d+|[а-я]+)\s*(?:день|дня|дней|сут|недел)/.test(t)||new RegExp(edge+"(?:на неделю|на выходные)"+tail).test(t);
    if(op.type==="SET_COOKING")return /готовить|без готовки|быстро|минимум готовки/.test(t);
    if(op.type==="SET_MODE")return /один магазин|в одном магазин|нескольк.*магаз|разным магазин|где дешевле/.test(t);
    return false
  }
