import fs from "node:fs";
import assert from "node:assert/strict";

const touch = fs.readFileSync("touch-layout-fix.css", "utf8");
const home = fs.readFileSync("votonobay-home-lock-v4.css", "utf8");

assert.match(touch, /votonobay-home-lock-v4\.css\?v=20260912-v2/);
assert.ok(
  touch.indexOf("votonobay-home-lock-v4.css") > touch.indexOf("votonobay-motion-guard.css"),
  "home authority must load after the historical visual layers"
);

// Desktop stays editorial: copy + Bay, then one primary decision entry and quiet self-service.
assert.match(home, /grid-template-areas:"copy bay" "actions bay" "self self"!important/);
assert.match(home, /grid-template-columns:minmax\(300px,520px\) max-content!important/);
assert.match(home, /content:"Что нужно решить\?"/);
assert.match(home, /\.v2-bay-first \.v2-hero-proof\{display:none!important\}/);
assert.match(home, /\.v2-bay-first \.v2-hero-bai\{[\s\S]*position:relative!important/);
assert.match(home, /\.v2-self-search \.v2-search\{[\s\S]*width:min\(680px,100%\)!important/);

// Touch layouts keep deliberate geometry and reduce the mascot to a peeking presence.
assert.match(home, /pointer:coarse/);
assert.match(home, /grid-template-areas:"copy" "bay" "actions" "self"!important/);
assert.match(home, /min-height:138px!important/);
assert.match(home, /overflow-x:auto!important/);
assert.match(home, /\.v2-bay-secondary\{[\s\S]*background:transparent!important/);
assert.doesNotMatch(home, /Сәлам|Салам|Там дешевле|Где дешевле/);

console.log("home visual lock tests: calm decision-first Home is protected");
