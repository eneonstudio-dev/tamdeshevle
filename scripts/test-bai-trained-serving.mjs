...
const fallback=fs.readFileSync(new URL('../bai-fallback-bridge.js',import.meta.url),'utf8');
...
const releaseLoad=fallback.indexOf('./config/bai-brain-release-bound.js');
const registryLoad=fallback.indexOf('./bai-brain-registry.js');
assert.ok(releaseLoad>=0&&registryLoad>releaseLoad,'staged release catalog must load before brain registry bootstrap');
...
console.log('Trained serving path passed: approved origin, auth proxy guards, deterministic release preload ordering, staged binding and disabled-by-default runtime registration are enforced.');
