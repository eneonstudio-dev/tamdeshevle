...
assert.match(css,/decision\/comparison surface owns its own visual system/i,"purchase CSS must leave the recommendation composition to the canonical comparison layer");
assert.doesNotMatch(css,/\.td-compare-hero\s*\{[^}]*background:/,"purchase CSS must not repaint Bay's recommendation hero");
assert.doesNotMatch(css,/linear-gradient\(135deg,#bfffdc,#2be487\)/i,"purchase overlay must not reintroduce the old bright-green comparison hero");
assert.match(comparisonCss,/html body \.td-compare-hero\{[^}]*background:radial-gradient/,"base comparison UI must own a dark premium recommendation hero");
assert.doesNotMatch(comparisonCss,/#f4f1e8|linear-gradient\(135deg,#24df84,#10a95e\)/i,"base comparison UI must not fall back to beige or full-green legacy surfaces");
assert.match(comparisonCss,/\.td-compare-why/,"base comparison UI must keep a dedicated rationale surface");
assert.match(comparisonCss,/#82e7ad/,"rationale deltas must remain visually distinct on the dark card");
assert.match(comparisonSource,/Решение Бая/);
assert.match(comparisonSource,/Как лучше купить/);
assert.match(comparisonSource,/Я бы взял этот вариант\./);
assert.match(comparisonSource,/ЕСЛИ ПРИОРИТЕТ ДРУГОЙ/);
assert.match(comparisonSource,/Почему этот вариант/);
assert.match(comparisonSource,/Votonobay не добавляет лишние товары/);
assert.doesNotMatch(comparisonSource,/Где действительно дешевле|Почему так дешевле|Ничего лишнего Бай не добавлял/,"base comparison source must not fall back to cheapest-only or obsolete copy");
...
console.log("Votonobay purchase experience passed: source copy → Bay-led decision → trust → rationale → chosen plan → truthful store handoff stays Votonobay-native without a cheapest-only fallback.");
