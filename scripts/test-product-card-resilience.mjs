...
assert(productUI.includes("tdCartQty"), "product cards must retain their current cart quantity for UI state");
...
assert(productUI.includes("≈ оценка"), "unverified prices must remain visibly approximate");
assert(productUI.includes("в корзине ·"), "catalog cards must show the current cart quantity");
assert(!productUI.includes("✓ подтверждено"), "product trust must not restore the rejected checkmark treatment");
assert(productUI.includes("rgba(43,228,135,.10)"), "product trust states must use the Votonobay mint treatment instead of the old beige UI");
...
console.log("Product card resilience checks passed: Votonobay trust states, in-cart feedback, aspect-safe images, retry/BFCache handling and narrow Android layout guards are wired.");
