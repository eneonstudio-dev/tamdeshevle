...
assert.deepEqual(result.plans[0].stores,["perek"],"optimizer must compare eligible single-store plans instead of pinning the current store");
assert.equal(result.plans[0].total,135);
assert.deepEqual(new Set(result.plans[0].products.map(line=>line.id)),new Set(["milk","bread"]),"winning one-store plan must preserve the canonical basket");
