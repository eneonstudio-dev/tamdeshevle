...
const emptyComparison = TDCompare.compare({ stores: stores.slice(0,2), products: verifiedProducts, cart: {}, city: 'msk', mode: 'walk', originStoreId: 'shop' });
assert(emptyComparison.every(row => row.total === null && row.complete === false && row.rankable === false), 'empty comparison rows must stay incomplete and unrankable');
