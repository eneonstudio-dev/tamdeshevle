...
assert.match(retailerSync,/if \(quality\.usable && verifiedStoreId && match\.comparison_eligible === true && match\.availability === "in_stock"\) \{[\s\S]*product\.prices/,'only usable, physically scoped verified overlays may mutate comparison prices');
...
console.log('Trust UI checks passed: regional estimates stay approximate, freshness-gated, outside ranking, and out of verified history.');
