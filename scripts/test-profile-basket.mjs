...
const windowObj={
  ...
  TDCompare:{
    ...
    goodsTotal(products,cart,storeId){return products.reduce((sum,p)=>sum+(Number(cart[p.id]||0)*Number(p.prices?.[storeId]||0)),0);}
  },
  addEventListener(){},
  dispatchEvent(){}
};
...
const first=api.snapshot();assert.equal(first.total,200);assert.equal(first.items,2);
...
assert.match(source,/Пока профиль хранится только на этом устройстве/,'local-only profile disclosure present');
assert.match(source,/один снимок корзины за день/,'daily snapshot disclosure present');
