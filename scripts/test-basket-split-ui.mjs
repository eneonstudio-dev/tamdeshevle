...
const roxy=fs.readFileSync('votonobay-roxy-split-decision-v1.js','utf8');
const roxyCss=fs.readFileSync('votonobay-roxy-split-decision-v1.css','utf8');
const purchase=fs.readFileSync('votonobay-roxy-purchase-skeleton-v1.js','utf8');

new Function(ui);new Function(roxy);
...
assert.match(purchase,/votonobay-roxy-split-decision-v1\.js/);
assert.match(roxy,/result\.worthSplitting/);assert.match(roxy,/travelKnown===false/);assert.match(roxy,/operationalCost==null/);
assert.match(roxy,/Пока не делю корзину/);assert.match(roxy,/Один магазин выгоднее по итогу/);
assert.match(roxy,/Номинальную разницу цен не выдаём за чистую экономию/);assert.match(roxy,/data-split-basket/);
assert.match(roxyCss,/\.roxy-split-decision/);assert.match(roxyCss,/@media\(max-width:520px\)/);
console.log('Basket split UI contract passed: honest one-vs-two store comparison, rejected split rationale, explicit shelf handoff, delivery fail-closed and Votonobay styling.');
