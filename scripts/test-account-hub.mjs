...
assert.match(src,/Сохранить в облако/);
assert.match(src,/Восстановить из облака/);
assert.match(src,/aria-modal/);
assert.match(src,/document\.body\.style\.overflow="hidden"/);
assert.match(src,/event\.key==="Escape"/);
assert.match(src,/window\.TDBai\?\.closePanel/);
assert.match(src,/requestAnimationFrame\(\(\)=>root\.querySelector\("\.td-account-close"\)\?\.focus\(\)\)/);
assert.doesNotMatch(src,/Следующий backend-слой/);
