...
assert(/<title>Votonobay — как лучше собрать корзину<\/title>/.test(html), "HTML title is legacy migration debt until browser-surface normalization lands");
...
assert(/Votonobay помогает понять, как лучше собрать корзину/.test(html), "existing HTML metadata positioning remains guarded until its reviewed Gate G migration slice");
assert(!/<title>[^<]*(Тамдешевле|Там дешевле|Проще)/.test(html), "retired master brand must not appear in the document title");
...
assert(/Votonobay — Бай/.test(favicon), "favicon title remains migration debt until its reviewed browser-surface slice");
assert(/#4FF59A/.test(favicon) && /#07100B/.test(favicon), "app icon must use the dark-and-mint palette");
assert(/Votonobay — на главную/.test(shell), "existing shopping-shell wordmark remains guarded until reviewed Gate G migration");
...
assert(/body\.td-votonobay/.test(css), "brand visual foundation must be scoped to the canonical body class");
...
assert(/Votonobay · Бай готовит главную/.test(coldStart), "current boot copy remains guarded until the reviewed Gate G browser migration slice");
...
assert(/Сэкономлено с Votonobay/.test(account) && /Подписка Votonobay Plus/.test(account), "current account copy remains migration debt until reviewed Gate G normalization");
assert(/tuneAccountBrand/.test(brand), "runtime brand layer must protect late account surfaces too");

console.log("Brand tests passed: canonical VOTONOBAI PWA identity is locked while remaining browser-visible legacy spelling stays explicit migration debt for reviewed Gate G normalization.");