(()=>{
  ...
  const MAX_HISTORY=8,MAX_TEXT=500,REQUEST_TIMEOUT_MS=9000,LOCAL_MODEL_MB=570;
  ...
  function sanitizeState(raw){
    ...
    return {
      budget:s.budget==null?null:Math.max(0,Number(s.budget)||0),currentTotal:Math.max(0,Number(s.currentTotal)||0),
      peopleCount:Math.max(1,Math.min(100,Number(s.peopleCount)||1)),duration:Math.max(1,Math.min(365,Number(s.duration)||1)),
      cookingPreference:String(s.cookingPreference||"normal").slice(0,32),mode:s.mode==="one"?"one":"multi",
  ...
  function localEnabled(){try{return localStorage.getItem("td_bai_local_model_enabled")==="1"}catch{return false}}

  async function ensureLocalRouter(){
    if(window.TDQwenRouter)return window.TDQwenRouter;
    localLoad=localLoad||import("./qwen-router.js?v=20260912-local-v1").catch(error=>{console.warn("[Bai Local Qwen] load failed",error);return null});
    await localLoad;return window.TDQwenRouter||null;
  }
  ...
  async function localRoute(text,history,baseline){
    ...
    const operations=safeOps(out.operations),reply=trim(out.reply);
    if(!operations.length&&!reply)return null;
    return {...baseline,ok:true,provider:"qwen-browser",operations,reply:reply||baseline?.reply||"",suggestions:[],agent:{version:"local-v1",model:String(out.model||"qwen-browser"),trace:["browser","policy"]}};
  }

  function offerLocal(baseline){
    if(!localSupported()||localEnabled())return baseline;
    const existing=(Array.isArray(baseline?.suggestions)?baseline.suggestions:[]).map(trim).filter(Boolean).filter(x=>x!==LOCAL_SUGGESTION).slice(0,2);
    const hasOps=Array.isArray(baseline?.operations)&&baseline.operations.length>0;
    const note=`Могу включить локальный нейро-режим: модель работает на устройстве, но первый запуск скачает около ${LOCAL_MODEL_MB} МБ.`;
  ...
  async function localControl(text,baseline){
    ...
    if(LOCAL_DISABLE.test(t)){
      ...
      lastStatus={attempted:true,used:true,provider:"qwen-browser",reason:"local_disabled",at:Date.now()};
      return {...baseline,operations:[],reply:"Локальный нейро-режим выключен. Бай снова использует серверный мозг и быстрые правила.",suggestions:[],provider:"qwen-browser-control"};
    }
    ...
    if(!localSupported()){
      ...
      return {...baseline,operations:[],reply:"На этом устройстве локальная нейромодель недоступна: нужен браузер с WebGPU. Обычный Бай продолжит работать.",suggestions:[],provider:"qwen-browser-control"};
    }
    const router=await ensureLocalRouter();router?.enable?.();
    lastStatus={attempted:true,used:true,provider:"qwen-browser",reason:"local_enabled",at:Date.now()};
    return {...baseline,operations:[],reply:`Нейро-режим включён. На первом сложном запросе браузер загрузит модель около ${LOCAL_MODEL_MB} МБ; дальше она работает локально на устройстве.`,suggestions:[],provider:"qwen-browser-control"};
  }
  ...
  async function route(text,history,baseline){
    ...
    if(local){lastStatus={attempted:true,used:true,provider:"qwen-browser",reason:"local_model",at:Date.now()};return local}
  ...
  function wrapBrain(brain){
    ...
    brain.route=async function(raw,history=[],...rest){const baseline=await original(raw,history,...rest);return route(raw,history,baseline)};
  ...
  function install(){
    ...
    let value;
    try{Object.defineProperty(window,"TDBaiBrain",{configurable:true,enumerable:true,get(){return value},set(next){value=wrapBrain(next)}});return true}catch{return false}
  }

  window.TDBaiAgentClient={
    install,route,shouldUse,sanitizeState,sanitizeCatalog,safeOps,
    enableLocal:async()=>{const router=await ensureLocalRouter();return router?.enable?.()||null},
    disableLocal:async()=>{const router=await ensureLocalRouter();return router?.disable?.()||null},
    status:()=>({...lastStatus,wrapped,configured:Boolean(window.TD_BAI_AGENT?.endpoint),localSupported:localSupported(),localEnabled:localEnabled(),localModelMB:LOCAL_MODEL_MB})
  };
  install();
})();
