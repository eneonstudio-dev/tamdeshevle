(function(){
  ...
  function renderBox(){
    const existing=document.querySelector(".td-sub-box");if(!window.state||state.screen!=="cart"){existing?.remove();return;}const list=suggestions(),signature=[state.storeId,...list.map(item=>`${item.original.id}:${item.alt.id}:${item.qty}:${item.save}`)].join("|");if(existing&&existing.dataset.signature===signature)return;existing?.remove();if(!list.length)return;ensureCss();
