(function(){
  ...
  function currentTotal(){
    ...
    let total=TDCompare.goodsTotal(PRODUCTS,state.cart||{},store.id,channel);
    if(store.kind==="delivery")total+=Number(store.delivery||0);
  ...
  function injectToday(){
    const existing=document.querySelector(".td-today-card");if(!window.state||state.screen!=="home"){existing?.remove();return;}const today=todayRecord();if(!today){existing?.remove();return;}const t=trend(today),store=getStore(today.storeId),wrap=document.querySelector("#app .wrap");if(!wrap)return;const html=`<div class="td-today-kicker">Цена моей корзины сегодня</div><div class="td-today-total">${money(today.total)}</div><span class="td-today-trend ${t.kind}">${esc(t.text)}</span><div class="td-today-meta">${today.items} товаров · ${esc(store?store.short:today.storeId)}${t.prev?` · было ${money(t.prev.total)}`:""}</div>`;let card=existing;if(!card){card=document.createElement("section");card.className="td-today-card";wrap.prepend(card);}if(card.innerHTML!==html)card.innerHTML=html;
  }
