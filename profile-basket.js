(function(){
  ...
  function currentTotal(){
    ...
    let total=quote.goods+fee.value;
  ...
  function snapshot(){
    ...
    const h=readJson(HISTORY_KEY,[]).filter(x=>x&&x.date!==record.date);
    h.push(record);h.sort((a,b)=>String(a.date).localeCompare(String(b.date)));
    writeJson(HISTORY_KEY,h.slice(-MAX_HISTORY));return record;
  }
  function history(){return readJson(HISTORY_KEY,[]).filter(x=>x&&x.verified===true&&Number.isFinite(Number(x.total))).sort((a,b)=>String(a.date).localeCompare(String(b.date)));}
  ...
  window.TDProfileBasket={profile,saveProfile,snapshot,history,stats,trend,dayKey,currentTotal,openProfile};
})();
