(function(){
  ...
  function modal(){
    document.querySelector(".td-auth-modal")?.remove();const root=document.createElement("div");root.className="td-auth-modal";root.innerHTML=`<div class="td-auth-card"><button class="td-auth-x" aria-label="Закрыть">×</button><div class="td-auth-logo">ТД</div><h2>Твой аккаунт</h2><p>Войди по email. Пришлём безопасную ссылку — пароль не нужен.</p>${configured()?`<form><input type="email" required autocomplete="email" inputmode="email" placeholder="you@example.com"><button type="submit">Получить ссылку</button></form><div class="td-auth-msg"></div>`:`<div class="td-auth-warning">Backend ещё не подключён. Нужны Project URL и publishable key.</div>`}</div>`;document.body.appendChild(root);root.querySelector(".td-auth-x").onclick=()=>root.remove();root.onclick=e=>{if(e.target===root)root.remove();};const form=root.querySelector("form");if(form)form.onsubmit=async e=>{e.preventDefault();const msg=root.querySelector(".td-auth-msg"),btn=form.querySelector("button"),email=form.querySelector("input").value;btn.disabled=true;msg.textContent="Отправляем…";try{await signInWithEmail(email);msg.textContent="Ссылка отправлена. Открой почту и вернись сюда.";}catch(err){msg.textContent=err.message==="EMAIL_INVALID"?"Проверь email.":"Не получилось: "+(err.message||err);}finally{btn.disabled=false;}};
  }
