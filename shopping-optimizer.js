(function(){
  ...
  function desired(s){
    ...
    return uniq(ids).filter(id=>{const p=all.find(x=>x.id===id);return p&&!excluded.includes(id)&&!excludedBrand(s,p)&&!existing.includes(p.name.toLowerCase().split(" ")[0])});
  }
  ...
  function topUp(lines,s,build,overhead=0){
    ...
    const excluded=s.excludedProducts||[],existingProducts=s.existingProducts||[];
    const target=targetBudget(s,overhead),used=new Set(lines.map(x=>x.id)),existing=existingProducts.join(" ").toLowerCase();const candidates=TDStoreAdapters.catalog().filter(p=>!used.has(p.id)&&!excluded.includes(p.id)&&!excludedBrand(s,p)&&!existing.includes(p.name.toLowerCase().split(" ")[0])).map(build).filter(Boolean).sort((a,b)=>a.price-b.price);let total=lines.reduce((n,x)=>n+x.price*x.quantity,0);for(const line of candidates){if(lines.length>=14||total>=target)break;if(total+line.price<=target){lines.push(line);total+=line.price}}return scaleQuantities(lines,s,overhead);
  }
  ...
  function eligibleStores(s){
    const city=window.state?.city||"msk",all=(typeof STORES!=="undefined"?STORES:[]).filter(x=>(x.city||[]).includes(city)&&x.kind!=="delivery");
    const requested=uniq(s.stores||[]);
    if(!requested.length)return all.map(x=>x.id);
    const scoped=all.filter(x=>requested.includes(x.id)).map(x=>x.id);
