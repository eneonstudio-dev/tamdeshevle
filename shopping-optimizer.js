(function(){
  ...
  function scaleOnly(lines,s,overhead=0){
    ...
    lines.forEach(line=>line.quantity=Math.max(1,Math.ceil(demand/Math.max(1,lines.length))));
    return lines;
  }
  function scaleQuantities(lines,s,overhead=0){
    ...
    let changed=true,rounds=0;while(changed&&rounds<80){changed=false;rounds++;for(const id of priority){const line=byId.get(id);if(!line)continue;const cap=Math.max(1,softCaps[id]||Math.ceil(demand/4));if(line.quantity>=cap)continue;if(total+line.price>target)continue;line.quantity++;total+=line.price;changed=true;}if(total>=target)break;}
    if(s.budget&&total<target*.72){const cheapest=[...lines].sort((a,b)=>a.price-b.price);let guard=0;while(total<target*.86&&guard++<80){let added=false;for(const line of cheapest){const absoluteCap=Math.max(2,Math.ceil(demand/2));if(line.quantity>=absoluteCap||total+line.price>target)continue;line.quantity++;total+=line.price;added=true;if(total>=target*.86)break;}if(!added)break;}}
    return lines;
  }
  ...
  function optimize(s){const allowed=s.stores.length?s.stores:[window.state?.storeId||"pyat"];const one=planOne(s,allowed[0]);if(s.selectionMode==="only"&&s.mode!=="multi")return[one];const multi=planMulti(s);return s.mode==="one"?[one]:[one,multi].sort((a,b)=>a.total-b.total);}
