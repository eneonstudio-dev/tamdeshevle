(function(){
  ...
  function targetBase(target){const a=Number(target?.amount)||0,u=target?.unit;if(!a)return null;if(u==="kg")return{kind:"mass",base:a*1000,label:`${a} кг`};if(u==="g")return{kind:"mass",base:a,label:`${a} г`};if(u==="l")return{kind:"volume",base:a*1000,label:`${a} л`};if(u==="ml")return{kind:"volume",base:a,label:`${a} мл`};if(u==="pcs")return{kind:"pcs",base:a,label:`${a} шт.`};if(u==="pack")return{kind:"pack",base:a,label:`${a} уп.`};return null}
  function applyTargets(lines,s){const targets=s.quantityTargets||{};for(const line of lines){const t=targetBase(targets[line.id]);if(!t)continue;let q=1;if(t.kind==="pack")q=Math.ceil(t.base);else{const p=packMeasure(line.pack);if(p&&p.kind===t.kind&&p.base>0)q=Math.ceil(t.base/p.base);else if(t.kind==="pcs"&&line.id==="eggs")q=Math.ceil(t.base/10);else q=Math.ceil(t.base)}line.quantity=Math.max(line.quantity||1,q);line.requestedMinQuantity=line.quantity;line.requestedAmountLabel=t.label;if(q>1)line.requestedAmountLabel+=` → ${q} уп.`}return lines}
