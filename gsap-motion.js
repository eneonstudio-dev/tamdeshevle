(function () {
  ...
  function loadV2Polish() {
    if (!document.querySelector('link[data-td-v2-polish]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = 'v2-polish.css?v=20260911-polish-v1';
      link.dataset.tdV2Polish = '1';
      document.head.appendChild(link);
    }
    if (!document.querySelector('script[data-td-v2-polish]')) {
      const script = document.createElement('script');
      script.src = 'v2-polish.js?v=20260911-polish-v1';
  ...
  function playV2() {
    if (reduce || !window.gsap || !window.state || state.screen !== "home") return false;
  ...
  window.addEventListener('td:v2-rendered', () => requestAnimationFrame(play));
  requestAnimationFrame(play);
})();
