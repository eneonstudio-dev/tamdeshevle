(function(){
  ...
  function enhanceHero(root){const hero=root.querySelector(".td-ai-bai");if(!hero||hero.dataset.visualV3)return;hero.dataset.visualV3="1";hero.querySelector("h2")?.replaceChildren(document.createTextNode("Что сегодня собираем?"));const p=hero.querySelector("p");if(p)p.textContent="Расскажи задачу как человеку. Бай поймёт бюджет, учтёт продукты дома и соберёт корзину.";const eyebrow=document.createElement("small");eyebrow.className="td-ai-v3-eyebrow";eyebrow.textContent="AI-АССИСТЕНТ ПОКУПОК";hero.insertBefore(eyebrow,hero.querySelector("h2"));const wrap=document.createElement("div");wrap.className="td-ai-v3-prompts";prompts.forEach(text=>{const b=document.createElement("button");b.type="button";b.textContent=text;b.onclick=()=>window.TDShoppingAssistant?.submit?.(text);wrap.appendChild(b)});hero.appendChild(wrap)}
  function enhanceLines(root){root.querySelectorAll(".td-ai-summary .td-ai-line").forEach((line,index)=>{const signature=line.textContent.trim();if(line.dataset.visualSignature===signature)return;line.dataset.visualSignature=signature;line.classList.add("td-ai-v3-product");line.style.setProperty("--card-order",String(Math.min(index,6)));line.querySelector(".td-ai-v3-photo")?.remove();const photo=photoFor(signature);const media=document.createElement("span");media.className="td-ai-v3-photo";if(photo){const img=document.createElement("img");img.src=photo;img.alt="";img.loading="lazy";img.decoding="async";img.onerror=()=>{img.remove();media.classList.add("fallback")};media.appendChild(img)}else media.classList.add("fallback");line.prepend(media)})}
