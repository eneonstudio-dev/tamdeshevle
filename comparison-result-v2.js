(()=>{
  ...
  function open(){
    ...
    const root=document.createElement("section");root.className="td-compare-v2";
    root.innerHTML=`<div class="td-compare-v2-shell"><header><button data-compare-close aria-label="Закрыть сравнение">←</button><div><small>Решение по корзине</small><b id="td-compare-v2-title">Как лучше купить</b></div></header><main>
      ...
      ${trustHTML(best)}
      <div class="td-compare-grid">${sorted.map((p,i)=>`<article class="td-compare-plan ${i===0?"best":""}"><div><small>${i===0?"Рекомендую":"Альтернатива"}</small><b>${title(p)}</b></div><strong>${money(p.total)}</strong><span>${(p.products||[]).length} товаров · ${storeCount(p)} ${storeCount(p)===1?"магазин":"магаз."}${p.convenienceCost?` · +${money(p.convenienceCost)} за разбиение`:""}</span><button data-compare-apply="${esc(p.id)}">${i===0?"Выбрать и продолжить":"Выбрать вариант"}</button></article>`).join("")}</div>
      <section class="td-compare-why"><h3>Почему этот вариант</h3>${moves.length?moves.slice(0,6).map(x=>`<div><span>${esc(x.emoji||"•")} ${esc(x.name)}<small>${esc(storeName(x.from))} → ${esc(storeName(x.storeId))}</small></span><b>${x.diff>0?`−${money(x.diff)}`:"лучше здесь"}</b></div>`).join(""):`<p>Решение учитывает общую стоимость корзины и количество магазинов. Votonobay не добавляет лишние товары.</p>`}</section>
      <div class="td-compare-note">Надёжность цен показывает качество уже имеющихся данных и не меняет расчёт. Подтверждённая экономия попадёт в профиль только если весь выбранный вариант состоит из свежих подтверждённых цен.</div>
    </main></div>`;
