(() => {
  ...
  const mem = () => window.TDBaiMemory || null;
  ...
  const PRESETS = {
    economy: {
      ...
      apply(base,required=this.required){
        ...
        s.requiredProducts=uniq([...(s.requiredProducts||[]),...required]); return s;
      },
      operations(base,required=this.required){const ops=[{type:"SET_MODE",value:"multi"},{type:"ADD_PREFERENCE",value:"budget"}];if(base.budget)ops.unshift({type:"CHANGE_BUDGET",value:Math.max(500,Math.round(base.budget*.78))});required.forEach(id=>ops.push({type:"REQUIRE",value:id}));ops.push({type:"REOPTIMIZE"});return ops;}
    },
    balanced: {
      title: "Сбалансированный",
      blurb: "нормальный запас еды без сильного перекоса в цену или удобство",
      required: ["chicken", "eggs", "bread", "banana", "apple", "water"],
      apply(base,required=this.required){const s=clone(base);s.preferences=uniq([...(s.preferences||[]),"balanced"]);s.requiredProducts=uniq([...(s.requiredProducts||[]),...required]);return s;},
      operations(base,required=this.required){const ops=[{type:"ADD_PREFERENCE",value:"balanced"}];required.forEach(id=>ops.push({type:"REQUIRE",value:id}));ops.push({type:"REOPTIMIZE"});return ops;}
    },
    easy: {
      title: "Без возни",
      blurb: "еда, которую можно быстро съесть или приготовить с минимумом действий",
      required: ["dumplings", "noodles", "eggs", "banana", "water"],
      apply(base,required=this.required){const s=clone(base);s.cookingPreference="minimal";s.preferences=uniq([...(s.preferences||[]),"convenience"]);s.requiredProducts=uniq([...(s.requiredProducts||[]),...required]);return s;},
      operations(base,required=this.required){const ops=[{type:"SET_COOKING",value:"minimal"},{type:"ADD_PREFERENCE",value:"convenience"}];required.forEach(id=>ops.push({type:"REQUIRE",value:id}));ops.push({type:"REOPTIMIZE"});return ops;}
    },
    hearty: {
      title: "Сытный",
      blurb: "больше основы и плотной еды, меньше пустых перекусов",
      required: ["chicken", "eggs", "buck", "pasta", "bread"],
      apply(base,required=this.required){const s=clone(base);s.preferences=uniq([...(s.preferences||[]),"hearty"]);s.requiredProducts=uniq([...(s.requiredProducts||[]),...required]);return s;},
      operations(base,required=this.required){const ops=[{type:"ADD_PREFERENCE",value:"hearty"}];required.forEach(id=>ops.push({type:"REQUIRE",value:id}));ops.push({type:"REOPTIMIZE"});return ops;}
    },
    healthy: {
      title: "Полегче",
      blurb: "больше обычной еды и фруктов, меньше случайных сладких добивок",
      required: ["chicken", "eggs", "buck", "apple", "banana", "water", "cottage"],
      apply(base,required=this.required){const s=clone(base);s.preferences=uniq([...(s.preferences||[]),"healthy"]);s.requiredProducts=uniq([...(s.requiredProducts||[]),...required]);return s;},
      operations(base,required=this.required){const ops=[{type:"ADD_PREFERENCE",value:"healthy"}];required.forEach(id=>ops.push({type:"REQUIRE",value:id}));ops.push({type:"REOPTIMIZE"});return ops;}
    }
  };

  function choosePresetIds(state, text){
    const t=low(text), prefs=(state.preferences||[]).join(" "), profile=mem()?.get?.()||{};
    ...
    if(profile.cooking==="minimal") return ["easy","balanced","economy"];
    if((profile.preferences||[]).includes("hearty")) return ["hearty","balanced","economy"];
    if((profile.preferences||[]).includes("healthy")) return ["healthy","balanced","economy"];
    if((profile.preferences||[]).includes("budget")) return ["economy","balanced","easy"];
  ...
  function build(state,text=""){
    ...
    const base=clone(state), ids=choosePresetIds(base,text), strategies=[], memory=mem();
    for(const id of ids){
      const preset=PRESETS[id], required=memory?.filterRequired?.(preset.required)||preset.required, scenario=preset.apply(base,required), plans=window.TDShoppingOptimizer.optimize(scenario)||[], best=plans[0];
      if(!best) continue;
      const strategy={
        ...
        productNames:(best.products||[]).slice(0,5).map(p=>p.name),productIds:(best.products||[]).map(p=>p.id).filter(Boolean),operations:preset.operations(base,required),score:score(id,base,text,best),actionText:`Выбираю вариант «${preset.title}»`
      };
      const memoryScore=memory?.scoreStrategy?.(strategy)||{delta:0,reasons:[]};strategy.score+=memoryScore.delta||0;strategy.memoryReasons=memoryScore.reasons||[];strategy.memoryAvoided=memoryScore.avoided||[];
  ...
  function explain(result,state){
    ...
    if(r.memoryReasons?.length)why+=` По твоим прошлым выборам: ${r.memoryReasons.join("; ")}.`;
  ...
  function afterChoice(chosen, alternatives, state){
    ...
    if(chosen.memoryReasons?.length)bits.push(`и это ближе к твоим привычкам: ${chosen.memoryReasons[0]}`);
