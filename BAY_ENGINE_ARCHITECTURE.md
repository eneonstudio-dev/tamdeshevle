...
**Character authority:** `BAI_CHARACTER_CANON.md`
**Related:** #468
...
The Bay Engine exists to make the **existing Bay** more intelligent, independent and useful. It is not permission to invent a new assistant personality. `BAI_CHARACTER_CANON.md` is binding across model/provider/framework changes.
...
| Character product canon | `BAI_CHARACTER_CANON.md` | Binding identity/decision philosophy for all future Brain/provider work |
...
Verified Result / PurchasePlan / safe state mutation
    ↓
Bay Response Composer (Character Canon; immutable facts stay immutable)
    ↓
Text / Voice TTS
```
...
**Character rule:** Bay first reaches and verifies a decision, then expresses it in character. Persona never substitutes for reasoning and never authors facts.
...
- Bay persona, Character Canon and response-policy rules;
...
- identify when a clarification is truly necessary;
- compare or explain already-verified alternatives using bounded Votonobay context;
...
## Character Canon contract

`BAI_CHARACTER_CANON.md` is the product authority for who Bay is. `BAI_CHARACTER_BIBLE.md` is legacy historical context only.

The engine must preserve these decision behaviours across providers:

- useful skepticism rather than blind acceptance of discounts/claims;
- dry care expressed through better outcomes, not empathy boilerplate;
- autonomy: safe reversible assumptions before unnecessary questions;
- real-value optimization, not lowest-price worship;
- willingness to choose a more expensive option when time/effort/risk make it better;
- willingness to disagree, say «не брать», or recommend a competitor when warranted;
- uncertainty proportional to evidence quality;
- serious mode without humour for payments, safety, health, personal data, major loss/error or visible distress;
- rare, non-mechanical humour/catchphrases;
- user sovereignty after a clear warning.

A model/provider migration that materially breaks these behaviours is a product regression even if raw benchmark/task accuracy improves.

## Persona contract

Persona does not belong to a provider.

Canonical Bay persona remains defined by Votonobay-owned rules/prompts/examples and the Character Canon, including tone, vocabulary, humour limits, seriousness mode, uncertainty behaviour and response length.
...
Reasoning/decision construction and response styling are separate stages. Character is how Bay expresses a good decision, not a substitute for reaching one.
...
In addition, durable behavioural memory must be correctable and must carry enough metadata to avoid turning one event into a permanent preference. Target semantics from the Character Canon are:
...
Bay does not optimize price alone. The decision layer must be able to represent, estimate or explicitly mark unknown the factors that can materially change a recommendation, including:

- money;
- travel/time;
- delivery;
- effort/number of stores;
- quality/fit where supported;
- probability of success / risk of missing stock;
- avoidable cooking or handling burden;
- waste risk;
- user preferences and past confirmed behaviour.

The exact scoring formula is not frozen here. What is frozen is the product rule: a slightly more expensive option may be the correct recommendation when the incremental savings do not justify the extra cost/effort/risk.

Any numeric decision must still be derived from verified/explicit inputs, not invented by the model.
...
Closed beta may use system/free STT/TTS. A branded paid voice is explicitly later work. Text and voice must reach the same orchestration/truth/action/Character Canon boundaries.
...
Run the same frozen shopping scenarios through both adapters and deterministic fallback. Verify persona stability, action equivalence where expected, truth rejection, decision-value behaviour, character regression and graceful failure.
...
2. The same canonical Bay persona policy and Character Canon apply across both.
...
11. Frozen character scenarios show that a provider change does not turn Bay into a generic assistant, comedian, salesperson, lowest-price optimizer or over-questioning interviewer.
12. Frozen decision scenarios include at least one case where a slightly more expensive option is correctly preferred because verified time/effort/risk outweigh the saving.
13. Memory behaviour does not promote a single observation into an unjustifiably permanent preference.
14. Serious-mode scenarios reliably suppress humour and casual character flourishes.
...
- redesigning Bay personality as part of a provider migration.
...
3. Character Canon / decision-behaviour stability;
4. deterministic structured/tool behaviour;
5. zero/low cost;
6. latency and availability;
7. privacy/data-policy fit;
