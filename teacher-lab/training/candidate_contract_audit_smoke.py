...
def safe_predictions():
    return [
        {
            'id':'a','intent':'build_basket','actions':[{'type':'set_constraint','payload':{'key':'budget','value':5000}}],
            'confidence':{'overall':'high','price':'unknown','availability':'unknown','quality':'unknown'}
        },
        {
            'id':'b','intent':'edit_basket','actions':[{'type':'remove_item','payload':{'product_id':'milk'}}],
            'confidence':{'overall':'high','price':'unknown','availability':'unknown','quality':'unknown'}
        },
    ]
...
assert safe['truth_boundary_violations']==0
...
print('Candidate contract audit smoke passed: payload-only actions, exact coverage, parse and truth-boundary failures fail closed.')
