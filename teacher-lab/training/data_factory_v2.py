...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('out'); args=ap.parse_args(); rows=build_tasks(); Path(args.out).write_text(''.join(json.dumps(x,ensure_ascii=False)+'\n' for x in rows),encoding='utf-8'); print(json.dumps(stats(rows),ensure_ascii=False,indent=2))
