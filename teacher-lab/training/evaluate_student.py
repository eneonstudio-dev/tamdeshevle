...
def load_model(config,adapter=None):
    ...
    base=config['base_model']; tok=AutoTokenizer.from_pretrained(base,trust_remote_code=True); tok.pad_token=tok.pad_token or tok.eos_token
...
def predict(tok,model,row,max_new_tokens):
    import torch
...
def main():
    ...
    target=Path(args.out); target.parent.mkdir(parents=True,exist_ok=True); ok=0
    with target.open('w',encoding='utf-8') as f:
        for i,row in enumerate(rows,1):
            try:
                result=predict(tok,model,row,args.max_new_tokens); result={'id':row.get('id'),**result}; ok+=1
            except Exception as exc:
                result={'id':row.get('id'),'parse_error':str(exc)}
            f.write(json.dumps(result,ensure_ascii=False)+'\n'); print(f'{i}/{len(rows)} ok={ok}',flush=True)
    print(json.dumps({'examples':len(rows),'parsed':ok,'predictions':str(target),'adapter':args.adapter or None},ensure_ascii=False))
