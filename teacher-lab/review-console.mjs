...
body{font:15px system-ui;margin:0;background:#f6f6f6;color:#111}header{position:sticky;top:0;background:#fff;border-bottom:1px solid #ddd;padding:12px;z-index:2}.wrap{max-width:1200px;margin:auto;padding:16px}.meta{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.card{background:#fff;border:1px solid #ddd;border-radius:10px;padding:12px;margin-bottom:12px}pre,textarea{white-space:pre-wrap;word-break:break-word;font:12px ui-monospace,monospace}textarea{width:100%;min-height:260px}.flags{font-weight:700}.request{font-size:18px;margin:8px 0}.actions{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}button{padding:9px 12px}button.sel{outline:3px solid #111}@media(max-width:800px){.grid{grid-template-columns:1fr}}
...
function render(){
  ...
  root.innerHTML='<section class="card"><div><b>'+escapeHtml(x.task_id)+'</b> · '+escapeHtml(x.status)+'</div><div class="request">'+escapeHtml(x.user_request||'')+'</div><div class="flags">'+escapeHtml(tags)+'</div><details><summary>Session context</summary><pre>'+escapeHtml(pretty(x.session_context||{}))+'</pre></details></section>'+
