...
body{font:15px system-ui;margin:0;background:#f6f6f6;color:#111}header{position:sticky;top:0;background:#fff;border-bottom:1px solid #ddd;padding:12px;z-index:2}.wrap{max-width:1200px;margin:auto;padding:16px}.meta{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.card{background:#fff;border:1px solid #ddd;border-radius:10px;padding:12px}pre,textarea{white-space:pre-wrap;word-break:break-word;font:12px ui-monospace,monospace}textarea{width:100%;min-height:260px}.flags{font-weight:700}.actions{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}button{padding:9px 12px}button.sel{outline:3px solid #111}.muted{color:#666}@media(max-width:800px){.grid{grid-template-columns:1fr}}
...
function render(){const x=current(); document.getElementById('progress').textContent=PACKET.length?`${'${index+1}'}/${'${PACKET.length}'}`:'0/0'; if(!x){root.innerHTML='<p>Нет кейсов.</p>';return} const d=decisions.get(x.task_id)||{}; root.innerHTML=`<section class="card"><div><b>${'${x.task_id}'}</b> · ${'${x.status}'}</div><div class="flags">${'${[...(x.conflicts||[]),...(x.flags||[])].join(" · ")||"без флагов"}'}</div></section><div class="grid"><section class="card"><h3>Left</h3><pre>${'${escapeHtml(pretty(x.left))}'}</pre></section><section class="card"><h3>Right</h3><pre>${'${escapeHtml(pretty(x.right))}'}</pre></section></div><div class="actions"><button data-d="approve_left">Approve left</button><button data-d="approve_right">Approve right</button><button data-d="approve_edited">Approve edited</button><button data-d="reject">Reject</button></div><label>Note <input id="note" style="width:100%" value="${'${escapeAttr(d.note||"")}' }"></label><div id="editBox" style="display:${'${d.decision==="approve_edited"?"block":"none"}'}"><h3>Edited target JSON</h3><textarea id="edited">${'${escapeHtml(d.editedText||pretty(x.left||x.right||{}))}'}</textarea></div>`; document.querySelectorAll('[data-d]').forEach(b=>{if(b.dataset.d===d.decision)b.classList.add('sel');b.onclick=()=>choose(b.dataset.d)});}
function choose(decision){const x=current(); const old=decisions.get(x.task_id)||{}; decisions.set(x.task_id,{...old,task_id:x.task_id,decision,note:document.getElementById('note')?.value||'',editedText:document.getElementById('edited')?.value||old.editedText});render();}
function escapeHtml(s){return String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))} function escapeAttr(s){return escapeHtml(s).replace(/"/g,'&quot;')}
document.getElementById('prev').onclick=()=>{if(index>0){saveDraft();index--;render()}}; document.getElementById('next').onclick=()=>{if(index<PACKET.length-1){saveDraft();index++;render()}};
