(function () {
  ...
  function pairOption(a, b, products, cart) {
    ...
    for (const product of entries) {
      ...
      allocations[chosen.store.id].push({ productId: product.id, name: product.name, quantity, unitPrice: chosen.price, lineTotal });
      total += lineTotal;
    }
  ...
  function optimize(options) {
    ...
    return { bestOne, bestTwo, extraSaving, worthSplitting: Boolean(bestOne && bestTwo && bestTwo.total < bestOne.total), pairCount: pairs.length };
  }
