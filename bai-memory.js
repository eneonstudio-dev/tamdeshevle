...
const blank=()=>({version:4,turns:0,preferences:[],productsLiked:[],productsAvoided:[],productSignals:{},preferenceSignals:{},outcomeSignals:{},cooking:null,usualBudget:null,usualPeople:null,storeMode:null,corrections:0,acceptedPlans:0,correctedPlans:0,pendingRecommendation:null});
let p;try{p={...blank(),...JSON.parse(localStorage.getItem(K)||"{}")}}catch{p=blank()}
p.productSignals=p.productSignals||{};p.preferenceSignals=p.preferenceSignals||{};p.outcomeSignals=p.outcomeSignals||{};p.version=4;
let history=[];
...
function undo(){if(!history.length)return save();p=history.pop();p.version=4;return save()}
...
function hint(){const b=[];if(p.cooking==="minimal")b.push("обычно без лишней готовки");if(p.usualPeople>1)b.push(`обычно на ${p.usualPeople}`);if(p.preferences.includes("budget"))b.push("цена важна");if(p.preferences.includes("hearty"))b.push("любишь сытнее");const n=avoidList().length;if(n)b.push(`${n} товар(а) больше не предлагаю автоматически`);if(p.correctedPlans)b.push(`учёл ${p.correctedPlans} правок моих корзин`);return b.slice(0,3).join(", ")}
function clear(){p=blank();history=[];try{localStorage.removeItem(K)}catch{}return p}
window.TDBaiMemory={get:()=>clone(p),learn,hint,clear,signal,outcome,productAffinity,shouldAvoid,avoidList,filterRequired,scoreStrategy,noteRecommendation,pending:()=>clone(p.pendingRecommendation),isFeedback:feedbackText};
