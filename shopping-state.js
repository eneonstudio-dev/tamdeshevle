(function(){
  ...
  function syncCart(){if(!window.state)return;state.cart=Object.fromEntries(state.products.filter(x=>x.sourceId).map(x=>[x.sourceId,x.quantity||1]));window.state.cart={...state.cart};try{const saved=JSON.parse(localStorage.getItem("td")||"{}");localStorage.setItem("td",JSON.stringify({...saved,cart:window.state.cart}))}catch{}window.render?.();}
