(function(){
  ...
  function apply(raw,providedOps){
    ...
    const s=TDShoppingState.get(),best=plans[0];TDShoppingState.syncCart();const uncertain=best.quality!=="LIVE",budget=s.budget,rest=budget?budget-best.total:null,totalUnits=best.products.reduce((n,x)=>n+(x.quantity||1),0),amounts=best.products.filter(x=>x.requestedAmountLabel).map(x=>`${x.name} — ${x.requestedAmountLabel}`).slice(0,3);
