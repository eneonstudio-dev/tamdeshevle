(function(){
  ...
  function apply(raw,providedOps){
    ...
    let message=s.selectionMode==="only"?`Понял: в корзине только ${best.products.map(x=>x.name).join(", ")||"выбранные товары"}. ${totalUnits} уп. — ${best.total}${budget?` из ${budget}`:""} ₽.`:`Собрал ${best.products.length} видов товаров, ${totalUnits} упаковок${s.peopleCount>1?` на ${s.peopleCount} человек`:""}${s.duration>1?` на ${s.duration} дн.`:""}${budget?` — ${best.total} из ${budget} ₽`:` — ориентир ${best.total} ₽`}.`;
