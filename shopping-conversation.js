(function(){
  ...
  function parse(raw){
    ...
    for(const [word,id] of Object.entries(storeWords))if(low.includes(word)){ops.push({type:"CHANGE_STORE",value:id});if(low.includes("только"))ops.push({type:"SET_MODE",value:"one"})}if(/из двух|нескольк.*магаз|там дешевле/.test(low))ops.push({type:"SET_MODE",value:"multi"});
  ...
  function apply(raw,providedOps){
    ...
    TDShoppingState.commit(ops[0]?.type||"UPDATE",s=>{
      for(const op of ops){
        ...
        if(op.type==="REMOVE_PRODUCT"){const id=op.value;if(id&&!s.excludedProducts.includes(id))s.excludedProducts.push(id);s.products=s.products.filter(x=>x.id!==id);s.requiredProducts=s.requiredProducts.filter(x=>x!==id);s.onlyProducts=s.onlyProducts.filter(x=>x!==id);}
        if(op.type==="REPLACE_PRODUCT"&&op.value){const from=op.value.from,to=op.value.to;if(from&&!s.excludedProducts.includes(from))s.excludedProducts.push(from);s.requiredProducts=s.requiredProducts.filter(x=>x!==from);s.onlyProducts=s.onlyProducts.filter(x=>x!==from);if(to&&!s.requiredProducts.includes(to))s.requiredProducts.push(to);if(s.selectionMode==="only"&&to&&!s.onlyProducts.includes(to))s.onlyProducts.push(to);s.excludedProducts=s.excludedProducts.filter(x=>x!==to);}
