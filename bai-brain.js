(() => {
  ...
  const emptyGoal=()=>({task:"basket",occasion:null,people:null,days:null,budget:null,cooking:null,preferences:[],requiredProducts:[],excludedProducts:[],onlyProducts:[],stores:[],mode:null});
  let memory={pending:null,lastIntent:"build",turns:0,lastSuggestions:[],goal:emptyGoal()};

  function products(text){const t=low(text),out=[];for(const [stem,id] of Object.entries(PRODUCTS)){const p=t.indexOf(stem);if(p>=0)out.push({id,pos:p})}return out.sort((a,b)=>a.pos-b.pos)}
  ...
  function mergeGoal(ops,text=""){
    ...
    for(const o of ops||[]){
      ...
      else if(o.type==="REMOVE_PRODUCT")memory.goal.excludedProducts=uniq([...(memory.goal.excludedProducts||[]),String(o.value)]);
      ...
      else if(o.type==="REPLACE_PRODUCT"&&o.value){memory.goal.excludedProducts=uniq([...(memory.goal.excludedProducts||[]),String(o.value.from)]);memory.goal.requiredProducts=uniq([...(memory.goal.requiredProducts||[]),String(o.value.to)])}
    }
    return memory.goal;
  }
  ...
  function question(text,options,kind,deferred=[]){mergeGoal(deferred);memory.pending={kind,deferred:dedupeOps(deferred)};memory.lastSuggestions=options||[];return{ok:true,provider:"bai-brain-v3",operations:[{type:"ASK_CLARIFICATION",value:text}],reply:text,suggestions:options||[],expectsAnswer:true,goal:{...memory.goal}}}
  function proposal(text,options){memory.lastSuggestions=options||[];return{ok:true,provider:"bai-brain-v3",operations:[],reply:text,suggestions:options||[],expectsAnswer:true,goal:{...memory.goal}}}
  function resolved(p,extra,reply){const operations=dedupeOps([...(p.deferred||[]),...extra,{type:"REOPTIMIZE"}]);mergeGoal(operations);return{operations,reply,suggestions:[],confirmedContext:true,goal:{...memory.goal}}}
  function resolvePending(raw){const p=memory.pending;if(!p)return null;const t=low(raw);memory.pending=null;
    ...
    if(no(raw))return{operations:[],reply:"Хорошо, это уточнение отменил. Уже понятые условия сохранил.",suggestions:[],confirmedContext:true,goal:{...memory.goal}};
  ...
  async function route(raw,history=[]){
    raw=String(raw||"").trim();if(!raw)return{ok:true,provider:"bai-brain-v3",operations:[],reply:"",goal:{...memory.goal}};
    memory.turns++;
    const pr=resolvePending(raw);if(pr)return{ok:true,provider:"bai-brain-v3",...pr};
    const t=low(raw),s=state(),ordered=products(raw),ids=uniq(ordered.map(x=>x.id)),shops=stores(raw),ops=[],advice=advisor(raw,s);if(advice)return advice;
    const b=budget(raw)||relativeBudget(raw,s.budget||memory.goal.budget),p=people(raw),d=days(raw);
    if(/верни как было|отмени последнее|назад/.test(t))return{ok:true,provider:"bai-brain-v3",operations:[{type:"UNDO"}],reply:"Откатываю последнее изменение.",suggestions:[],goal:{...memory.goal}};
    ...
    if(ops.length){if(!ops.some(o=>o.type==="REOPTIMIZE"))ops.push({type:"REOPTIMIZE"});mergeGoal(ops,t);let reply="Понял, обновляю корзину и сохраняю предыдущие условия.";if(ops.some(o=>o.type==="REMOVE_PRODUCT"))reply="Убрал. Остальные условия не трогаю.";else if(ops.some(o=>o.type==="ADD_PRODUCT"))reply="Добавил. Остальную корзину сохраняю.";else if(ops.some(o=>o.type==="REPLACE_PRODUCT"))reply="Понял, именно замена. Остальное оставляю как было.";else if(ops.some(o=>o.type==="SET_ONLY_PRODUCTS"))reply="Принял: только эти продукты, без лишнего.";else if(/только бюджет|только цену/.test(t)&&b)reply="Меняю только бюджет. Остальную цель не трогаю.";else if(ops.some(o=>o.type==="SET_MODE"&&o.value==="one"))reply="Понял: собираю в одном магазине, остальные условия сохраняю.";else if(b||p||d)reply="Принял новые условия. Пересчитываю текущий вариант.";const next=[];if(!s.budget&&!memory.goal.budget)next.push("Задать бюджет");if(!(s.preferences||[]).length&&!memory.goal.preferences.length&&!hasStyle)next.push("Сделать сытнее");if(!s.cookingPreference&&!memory.goal.cooking)next.push("Минимум готовки");return{ok:true,provider:"bai-brain-v3",operations:dedupeOps(ops),reply,suggestions:next.slice(0,3),expectsAnswer:false,goal:{...memory.goal}}}
