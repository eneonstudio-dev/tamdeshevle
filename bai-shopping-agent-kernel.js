(()=>{
  ...
  function verifyActions(actions,before,after,agent=session){const errors=[],b=itemMap(before?.products||[]),a=itemMap(after?.products||[]),build=actions.some(x=>x.type==="rebuild_basket"||(x.type==="set_constraint"&&x.payload?.key==="intent"&&x.payload?.value==="build"));for(const x of actions){const p=x.payload||{};if(x.type==="add_item"&&!build&&!a.has(p.product_id))errors.push({code:"ADD_NOT_APPLIED",product_id:p.product_id});if(x.type==="remove_item"&&a.has(p.product_id))errors.push({code:"REMOVE_NOT_APPLIED",product_id:p.product_id});if(x.type==="replace_item"&&(a.has(p.from_product_id)||!a.has(p.to_product_id)))errors.push({code:"REPLACE_NOT_APPLIED",from:p.from_product_id,to:p.to_product_id});if(x.type==="change_quantity"){const old=Number(b.get(p.product_id)?.quantity)||0,next=Number(a.get(p.product_id)?.quantity)||0,expected=Number.isFinite(Number(p.quantity))?Number(p.quantity):Number.isFinite(Number(p.delta))?Math.max(0,old+Number(p.delta)):next;if(next!==expected)errors.push({code:"QUANTITY_NOT_APPLIED",product_id:p.product_id,expected,actual:next})}if(x.type==="set_constraint"){const key=p.key,checks={budget:()=>Number(agent.budget)===Number(p.value),people_count:()=>Number(agent.constraints.people_count)===Number(p.value),duration_days:()=>Number(agent.constraints.duration_days)===Number(p.value),cooking:()=>agent.constraints.cooking===String(p.value),healthy:()=>agent.constraints.healthy===Boolean(p.value),excluded_brand:()=>agent.constraints.excluded_brands.some(x=>low(x)===low(p.value)),excluded_product:()=>agent.constraints.excluded_products.includes(String(p.value)),required_category:()=>agent.constraints.required_categories.some(x=>low(x)===low(p.value)),existing_product:()=>agent.constraints.existing_products.some(x=>low(x)===low(p.value)),preference:()=>agent.preferences.some(x=>low(x)===low(p.value)),store_mode:()=>agent.store_constraints.mode===String(p.value),store_limit:()=>agent.store_constraints.limit===(p.value==null?null:Number(p.value)),store_ids:()=>JSON.stringify([...agent.store_constraints.store_ids].sort())===JSON.stringify(p.value.map(String).sort()),delivery_mode:()=>agent.delivery_constraints.mode===String(p.value),delivery_deadline:()=>agent.delivery_constraints.deadline===String(p.value),delivery_max_fee:()=>agent.delivery_constraints.max_fee===(p.value==null?null:Number(p.value)),intent:()=>agent.intent===String(p.value),only_products:()=>true,user_note:()=>true};if(checks[key]&&!checks[key]())errors.push({code:"CONSTRAINT_NOT_APPLIED",key})}}
    return{ok:errors.length===0,errors};
  }
  function replyFor(actions,before,after){const names=id=>product(id)?.name||id,diff=[],present=itemMap(after?.products||[]);for(const a of actions){const p=a.payload||{};if(a.type==="add_item"&&present.has(p.product_id))diff.push(`Добавил ${names(p.product_id)}.`);if(a.type==="remove_item")diff.push(`Убрал ${names(p.product_id)}.`);if(a.type==="replace_item")diff.push(`Заменил ${names(p.from_product_id)} на ${names(p.to_product_id)}.`);if(a.type==="change_quantity")diff.push(`Изменил количество ${names(p.product_id)}.`)}const total=Number(after?.currentTotal)||0;return`${diff.join(" ")||"Корзина проверена и обновлена."}${(after?.products||[]).length?` Сейчас ${(after.products||[]).length} позиций, итог ≈ ${Math.round(total)} ₽.`:""}`}
  ...
  function recovery(value){const code=String(value?.error?.code||value?.code||"UNKNOWN"),map={UNKNOWN_PRODUCT:["Не нашёл такой товар в текущем каталоге.",["Показать похожие","Оставить корзину"]],ITEM_NOT_IN_BASKET:["Этого товара уже нет в корзине.",["Показать корзину"]],INVALID_REPLACEMENT:["Так заменить не получилось — старый товар оставил.",["Подобрать замену","Оставить как есть"]],INVALID_QUANTITY:["Количество должно быть больше нуля и не больше 99.",["Оставить как есть"]],INVALID_STORE_IDS:["Такого магазина нет среди доступных.",["Показать магазины"]],CONSTRAINT_VIOLATION:["Условия конфликтуют с корзиной. Изменение отменил.",["Ослабить ограничение","Оставить как есть"]],EFFECT_NOT_VERIFIED:["Не смог подтвердить изменение, поэтому вернул корзину как было.",["Попробовать ещё раз","Оставить как есть"]],NO_ACTION:["Не понял, что именно изменить в покупке.",["Показать корзину"]],PROVIDER_UNAVAILABLE:["Нейро-режим сейчас недоступен, но простые команды продолжают работать.",["Включить нейро-режим (~310 МБ)","Показать корзину"]],OUT_OF_SCOPE:["Я занимаюсь покупками: товарами, корзиной, магазинами и доставкой.",["Собрать корзину","Найти товар"]]};const [message,suggestions]=map[code]||["Не получилось безопасно выполнить действие. Корзина не изменилась.",["Попробовать ещё раз","Показать корзину"]];return{message,suggestions,code}}
  ...
  function execute(actions,{input=""}={}){
    ...
    if(!effect.ok||!invariants.ok){try{window.TDShoppingState?.undo?.();window.TDShoppingState?.syncCart?.();window.render?.();syncFromLegacy(legacy(),beforeSession)}catch{save(beforeSession)}return error(!effect.ok?"EFFECT_NOT_VERIFIED":"CONSTRAINT_VIOLATION","Изменение не подтверждено и было отменено.",{effects:effect.errors,constraints:invariants.failures})}
  ...
  window.TDBaiShoppingAgentKernel={version:VERSION,actions:[...ACTIONS],domainGate,propose,validateAction,validateState,verifyActions,execute,run,recovery,state:{get:snapshot,save,reset,syncFromLegacy},_test:{legacyToActions,toLegacy,projectSession,normalize,hash}};
})();
