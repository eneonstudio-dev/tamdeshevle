...
Between behavior head `5893599c...` and Gate-F decision head `0e605387...`, the repository changed only teacher/training workflow/code plus `PROJECT_STATUS.md` and `SECURITY_GATE_F.md`. No shopping runtime, truth, handoff, browser release-scope, auth/data-boundary or retailer execution file changed. The final replay PR still reruns release-relevant CI on its own merged view instead of relying on that inheritance alone.
...
- `scripts/test-retailer-handoff-gate.py`, `scripts/test-roxy-handoff.py` and Sara 2 release acceptance guard usable targets and honest copy.
